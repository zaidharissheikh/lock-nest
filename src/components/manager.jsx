...
const Manager = () => {
  ...
  const savePassword = async () => {
    if (form.url.length > 3 && form.username.length >= 3 && form.password.length >= 6) {
      ...
      const passwordData = {...form, id: uuid4()};
      setPasswordArr([...passwordArr, passwordData]);
  ...
  const editPassword = (id) => {
    ...
    setForm({ url: edited.url, username: edited.username, password: edited.password, id: id });
                ...
                {passwordArr.map((item)=>{
                    return (
                        ...
                        <td className="px-2 md:px-4 py-2 md:py-3 flex items-center text-xs md:text-base max-w-[80px] md:max-w-none">
                          <span className="truncate mr-1">{"•".repeat(item.password.length)}</span>
